    ...
    try {
        ...
        if (file.mimetype.startsWith('image/')) {
            ...
            sharp(file.path)
                ...
                .toBuffer(async (err, buffer) => {
                    if (err) {
                        return res
                            .status(500)
                            .send('Error processing image: ' + err.message)
                    }
            ...
            */
            ...
            res.status(200).send({
                message: 'Text or HTML uploaded successfully',
            ...
            if (file.size > MAX_FILE_SIZE) {
                return res
                    .status(400)
                    .json({ error: 'File exceeds 150 MB limit' })
            }
...
const uploadAgreementPDF = expressAsyncHandler(async (req, res) => {
    try {
        ...
        if (!html || !fullName) {
            return res
                .status(400)
                .json({ message: 'HTML and full name are required' })
        }
        ...
        //     return res
        //         .status(400)
        //         .json({ message: 'Agreement must be accepted' })
        // }
        ...
        const cleanedHtml = html
            ...
            console.log('Cleaned HTML for PDF generation:', cleanedHtml)
        ...
        try {
            ...
            await page
                .waitForSelector('img[src^="data:image"]', {
                    timeout: 5000,
                })
                .catch(() => {
        ...
        console.log('PDF generated successfully, size:', pdfBuffer)
        // 🧾 Unique key
...
// User app
const uploadFileToPocketBase = expressAsyncHandler(async (req, res) => {
    try {
        ...
        if (!title && !htmlContent) {
            return res
                .status(400)
                .send({ message: 'Please provide title and htmlContent' })
        }
...
const uploadHTMLToPocketBase = expressAsyncHandler(async (req, res) => {
    try {
        const pb = await getPocketbaseClient()
        await authenticateAdmin(pb)
        ...
        if (!title && !uploadedFile) {
            return res
                .status(400)
                .send({ message: 'Please provide title and file' })
        }
...
// User & Admin
const getPbFile = expressAsyncHandler(async (req, res) => {
    try {
    ...
    async (req, res) => {
        ...
        if (
            ...
            return res.status(400).json({
                message:
                    'Missing required fields: user_id, article_id, improvement_id , htmlContent, title',
            })
        }
...
// User & Admin
const getIMPFile = expressAsyncHandler(async (req, res) => {
    try {
        const { recordid, articleRecordId } = req.query

        if (!recordid && !articleRecordId) {
            res.status(400).json({
                message: 'Invalid request: missing recordid or articleRecordId',
            })
            return
        }
        let result
...
const publishImprovementFileFromPocketbase = expressAsyncHandler(
    async (req, res) => {
        ...
        if (!record_id || !article_id) {
            return res.status(400).json({
                message: 'Missing required fields: record_id, article_id',
            })
        }
        ...
        try {
            ...
            try {
                improvementRecord = await pb
                    .collection('edit_requests')
                    .getOne(record_id)
            } catch (err) {
                console.log(err)
                return res.status(200).json({
                    message: 'Improvement record has no HTML file to publish',
                })
            }
            ...
            const fileUrl = pb.files.getUrl(
                improvementRecord,
                improvementRecord.edited_html_file,
                { download: true }
            )
            ...
            console.log(
                '❌ Error publishing improvement file from pocketbase:',
                err
            )
...
const deleteImprovementRecordFromPocketbase = expressAsyncHandler(
    async (req, res) => {
        ...
        if (!record_id) {
            return res
                .status(400)
                .json({ message: 'Missing required fields: record_id' })
        }
        try {
            const pb = await getPocketbaseClient()
            await authenticateAdmin(pb)
            ...
            console.log(
                'Error deleteing improvement file from pocketbase:',
                err
            )
            return res.status(500).json({ message: 'Internal server error' })
        }
