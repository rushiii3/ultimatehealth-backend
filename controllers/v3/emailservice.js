...
const resendVerificationEmail = expressAsyncHandler(async (req, res) => {
    ...
    const successMessage =
        'If account exists, verification email has been sent.'
...
const verifyEmail = expressAsyncHandler(async (req, res) => {
    ...
    if (role !== ROLES.USER && role !== ROLES.DOCTOR) {
        {
            throwError(
    ...
    // Skipping this as we are going to have a diff endpoint for verification during signature

    // if (role === ROLES.MODERATOR || role === ROLES.ADMIN) {
    //     // Skipping this as we are going to have a diff endpoint for verification during signature

    //     // const adminUser = await Admin.exists({ email })
