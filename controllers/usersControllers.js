...
const {
    blackListToken,
    addTokenToBlacklist,
} = require('../services/db/dbTokenService')
const {
    findArticleById,
...
module.exports.sendOTPForForgotPassword = expressAsyncHandler(
    async (req, res) => {
        ...
        const cooldownMs =
            Number(process.env.OTP_RESEND_COOLDOWN_SECONDS) * 1000
        if (
...
module.exports.verifyOtpForForgotPassword = expressAsyncHandler(
    async (req, res) => {
        ...
        const isPasswordSame = await isSamePassword(
            newPassword,
            account.password
        )
...
module.exports.getSocials = expressAsyncHandler(async (req, res) => {
    ...
    if (Number(type) === 1) {
        ...
        if (author.followings) {
            author.followings = author.followings.filter(
                (user) => user !== null
            )
        }
        sendSuccess(
...
module.exports.updateProfileImage = expressAsyncHandler(async (req, res) => {
    ...
    const updatedUser = await updateUserProfilePictureById(
        user._id,
        profileImageUrl
    )
...
module.exports.updateUserGeneralDetails = expressAsyncHandler(
    async (req, res) => {
        ...
        const updatedUserGeneralDetailsById =
            await updateUserGeneralDetailsById(userId, {
                user_name: username,
...
module.exports.updateUserContactDetails = expressAsyncHandler(
    async (req, res) => {
        ...
        const updatedUserContactDetailsById =
            await updateUserContactDetailsById(req.user.userId, {
                contact_detail: {
                    email_id: email,
                    phone_no: phone,
                },
...
module.exports.updateUserPassword = expressAsyncHandler(async (req, res) => {
    ...
    const updatedUserPassword = await updateUserPasswordById(
        user._id,
        new_password
    )
    ...
    async (req, res) => {
        try {
            ...
            if (!Array.isArray(contentClusters)) {
                return res
                    .status(400)
                    .json({ error: 'contentClusters must be an array' })
            }
    ...
    async (req, res) => {
        try {
            const user = await User.findById(req.userId).populate(
                'notificationPreferences.contentClusters'
            )
            if (!user) return res.status(404).json({ error: 'User not found' })
